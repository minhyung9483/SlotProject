...
<%@ page import="com.slot.Model.NaverPlaceSlot" %>
...
<%
	...
	final String TAG = "NAVER_PLACE_SLOTS_PAGE";
	...
	List<NaverPlaceSlot> slotList = DBConnector.getNaverPlaceSlotList(0, setPage - 1, m_SearchType, m_SearchValue, m_OrderType, member.getUSER_PERM(), member.getUSER_IDX());
					...
					<div class="card-body p-4">
						<div class="row d-flex align-items-center">
							...
							<div class="col-lg-2 d-flex align-items-center justify-content-end justify-content-lg-start mt-3 px-2 mb-lg-0 px-lg-0 ps-lg-1 pe-lg-2">
								...
								<a href="/naver-place-slots" type="button" class="btn btn-outline-primary ms-2" _msttexthash="9814740" _msthash="28"><i class="ti ti-reload fs-6"></i></a>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="d-flex align-items-stretch">
				<div class="card w-100">
					<div class="card-body p-4">
						<div class="row">
							<div class="col-lg-4">
								<h5 class="card-title fw-semibold mb-4 col-lg-12">네이버 플레이스<%=totalCount>0?"("+totalCount+")":""%></h5>
							</div>
							...
							<table class="table text-nowrap mb-0 align-middle">
								...
								<tr>
									...
									<%
										...
										if("M".equals(member.getUSER_PERM())){
											Title = new String[]{"번호", "상태", "아이디", "슬롯타입", "키워드", "URL", "플레이스명", "플레이스코드", "시작일", "종료일", "관리"};

										}else{
											Title = new String[]{"번호", "아이디", "슬롯타입", "키워드", "URL", "플레이스명", "플레이스코드", "시작일", "종료일", "관리"};
								...
								<tr>
									...
									<td class=""><%--플레이스링크--%>
										<input type="text" class="form-control text-center" id="URL_<%=slotList.get(i).getSLOT_IDX()%>" name="URL_<%=slotList.get(i).getSLOT_IDX()%>" value="<%=!"".equals(slotList.get(i).getPLCE_URL()) ? slotList.get(i).getPLCE_URL() : ""%>" title="<%=!"".equals(slotList.get(i).getPLCE_URL()) ? slotList.get(i).getPLCE_URL() : ""%>">
									</td>
									<td class=""><%--묶음MID--%>
										<input type="text" class="form-control text-center" id="NAME_<%=slotList.get(i).getSLOT_IDX()%>" name="NAME_<%=slotList.get(i).getSLOT_IDX()%>" value="<%=!"".equals(slotList.get(i).getPLCE_NAME()) ? slotList.get(i).getPLCE_NAME() : ""%>" title="<%=!"".equals(slotList.get(i).getPLCE_NAME()) ? slotList.get(i).getPLCE_NAME() : ""%>">
									</td>
									<td class=""><%--단품MID--%>
										<input type="text" class="form-control text-center" id="CODE_<%=slotList.get(i).getSLOT_IDX()%>" name="CODE_<%=slotList.get(i).getSLOT_IDX()%>" value="<%=!"".equals(slotList.get(i).getPLCE_CODE()) ? slotList.get(i).getPLCE_CODE() : ""%>" title="<%=!"".equals(slotList.get(i).getPLCE_CODE()) ? slotList.get(i).getPLCE_CODE() : ""%>" >
									</td>
									<td class=""><%--시작일--%>
...
//				System.out.println("*********************************");
			...
			<nav class="d-flex justify-content-between pt-3" aria-label="Page navigation">
				<ul class="pagination">
					<li class="page-item">
						<% 	if(setPage > 1){ %>
						<a class="page-link d-flex border-0" href="/naver-place-slots/<%=setPage - 1%><%=Param%>">
				...
				<ul class="pagination d-none d-sm-block d-sm-flex">
					<%
						if(prev){
							out.print("<li class=\"page-item \"><a class=\"page-link\" href=\"/naver-place-slots/"+(startPage - 10)+Param+"\"> << </a></li>");
						}
						for(int i=startPage; i<=endPage; i++){
							out.print("<li class=\"page-item "+ (i==setPage ? "active" : "") +" \"><a class=\"page-link\" href=\"/naver-place-slots/"+(i)+Param+"\">"+(i)+"</a></li>");
						}
						if(next){
							out.print("<li class=\"page-item \"><a class=\"page-link\" href=\"/naver-place-slots/"+(startPage + 10)+Param+"\"> >> </a></li>");
						}
				...
				<ul class="pagination">
					<li class="page-item">
						...
						<a class="page-link d-flex border-0" href="/naver-place-slots/<%=setPage + 1%><%=Param%>" aria-label="Next">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="updateForm" name="updateForm">
					...
					<div class="mb-3">
						<label for="PLCE_NAME_UP" class="col-form-label">묶음MID</label>
						<input type="text" class="form-control PLCE_NAME_UP" id="PLCE_NAME_UP" name="PLCE_NAME_UP">
					</div>
					<div class="mb-3">
						<label for="PLCE_CODE_UP" class="col-form-label">단품MID</label>
						<input type="text" class="form-control PLCE_CODE_UP" id="PLCE_CODE_UP" name="PLCE_CODE_UP">
					</div>
					<div class="mb-3">
						<label for="PLCE_KYWD_UP" class="col-form-label">키워드</label>
						<input type="text" class="form-control PLCE_KYWD_UP" id="PLCE_KYWD_UP" name="PLCE_KYWD_UP">
					</div>
					<div class="mb-3">
						<label for="PLCE_URL_UP" class="col-form-label">플레이스링크</label>
						<input type="text" class="form-control PLCE_URL_UP" id="PLCE_URL_UP" name="PLCE_URL_UP">
					</div>
				</form>
			</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="updatesForm" name="updatesForm">
					...
					<div class="mb-3">
						<label for="PLCE_NAME_UPS" class="col-form-label">플레이스명</label>
						<input type="text" class="form-control PLCE_NAME_UPS" id="PLCE_NAME_UPS" name="PLCE_NAME_UPS">
					</div>
					<div class="mb-3">
						<label for="PLCE_CODE_UPS" class="col-form-label">플레이스코드</label>
						<input type="text" class="form-control PLCE_CODE_UPS" id="PLCE_CODE_UPS" name="PLCE_CODE_UPS">
					</div>
					<div class="mb-3">
						<label for="PLCE_KYWD_UPS" class="col-form-label">키워드</label>
						<input type="text" class="form-control PLCE_KYWD_UPS" id="PLCE_KYWD_UPS" name="PLCE_KYWD_UPS">
					</div>
					<div class="mb-3">
						<label for="PLCE_URL_UPS" class="col-form-label">URL</label>
						<input type="text" class="form-control PLCE_URL_UPS" id="PLCE_URL_UPS" name="PLCE_URL_UPS">
					</div>
				</form>
			</div>
			<div class="modal-footer justify-content-center">
			...
			<div class="modal-body">
				<form id="uploadForm" name="uploadForm">
					<div class="mb-3 text-end">
						<button type="button" class="btn btn-outline-primary ms-2 mb-1" onclick="location.href='/files/Excel/naverplace_sample.xlsx'">
							네이버 플레이스 대량등록 양식 <i class="ti ti-download"></i>
						</button>
					</div>
					<div class="mb-3">
						<label for="uploadFile" class="col-form-label">네이버 플레이스 엑셀 업로드</label>
					...
					<div class="mb-3">
						...
						<table class="table text-nowrap mb-0 align-middle">
							...
							<tr>
								<td class=""><%=naverPlaceSlotTypeList.get(i).getNP_SLOT_TYPE_IDX()%></td>
								<td class="border-start"><%=naverPlaceSlotTypeList.get(i).getTYPE_NAME()%></td>
							</tr>
							<%}%>
							</tbody>
						</table>
...
<script>
	...
	updateModal.addEventListener('show.bs.modal', function(event) {
		...
		var PLCE_NAME = button.getAttribute('data-bs-gid');
		var PLCE_CODE = button.getAttribute('data-bs-mid');
		var PLCE_KYWD = button.getAttribute('data-bs-kywd');
		var PLCE_URL = button.getAttribute('data-bs-link');
		...
		var PLCE_NAMEInput = updateModal.querySelector('.modal-body .PLCE_NAME_UP');
		var PLCE_CODEInput = updateModal.querySelector('.modal-body .PLCE_CODE_UP');
		var PLCE_KYWDInput = updateModal.querySelector('.modal-body .PLCE_KYWD_UP');
		var PLCE_URLInput = updateModal.querySelector('.modal-body .PLCE_URL_UP');
		...
		PLCE_NAMEInput.value = PLCE_NAME;
		PLCE_CODEInput.value = PLCE_CODE;
		PLCE_KYWDInput.value = PLCE_KYWD;
		PLCE_URLInput.value = PLCE_URL;
...
<script>
	...
	function setDelete() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				location.href = "/naver-place-slots"+json.url;
			}
		});
	}
	...
	function setUpdateOne(USER_IDX_UP, SLOT_IDX_UP) {
		...
		json.PLCE_NAME = $("#NAME_"+SLOT_IDX_UP).val();
	...
	function setUpdates() {
		...
		checkbox.each(function(i) {
			...
			json.USER_IDX = USER_IDX_SLOT_IDX[0];
			json.SLOT_IDX = USER_IDX_SLOT_IDX[1];
			json.PLCE_NAME = $("#NAME_"+USER_IDX_SLOT_IDX[1]).val();
	...
	function setUpdatesTogether() {
		...
		checkbox.each(function(i) {
			...
			json.USER_IDX = USER_IDX_SLOT_IDX[0];
			json.SLOT_IDX = USER_IDX_SLOT_IDX[1];
			json.PLCE_NAME = $("#PLCE_NAME_UPS").val();
	...
	function setUpload() {
		...
		$.ajax({
			...
			success: function(response) {
				...
				location.href = "/naver-place-slots";
			}
		});
	}
