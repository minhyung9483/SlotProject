...
<%@ page import="com.slot.Model.NaverPlaceLogSlot" %>
<!doctype html>
<html lang="ko">
...
<%
	...
	final String TAG = "NAVER_PLACE_LOG_SLOT_PAGE";
	...
	int totalCount  = DBConnector.getNaverPlaceLogSlotListTotalCount(m_SearchType, m_SearchValue, m_StartDate, m_EndDate, member.getUSER_PERM(), member.getUSER_IDX());
	List<NaverPlaceLogSlot> logSlotList = DBConnector.getNaverPlaceLogSlotList(setPage - 1, m_SearchType, m_SearchValue, m_StartDate, m_EndDate, member.getUSER_PERM(), member.getUSER_IDX());
...
<script type="text/javascript">
	...
	function makeExcel(jsonResult, jsonSearch){
		...
		if(jsonSearch.SearchValue != ""){
			...
			} else if (jsonSearch.SearchType == "PLCE_NAME") {
				jsonSearch.SearchType = "플레이스명";
			} else if (jsonSearch.SearchType == "PLCE_CODE") {
				jsonSearch.SearchType = "플레이스코드";
			} else if (jsonSearch.SearchType == "PLCE_KYWD") {
				jsonSearch.SearchType = "키워드";
			} else if (jsonSearch.SearchType == "PLCE_URL") {
					...
					<div class="card-body p-4">
						<div class="row d-flex align-items-center">
							...
							<div class="col-lg-2 d-flex align-items-center justify-content-end justify-content-lg-start mt-3 px-2 mb-lg-0 px-lg-0 ps-lg-1 pe-lg-2">
								...
								<a href="/naver-place-log-slots" type="button" class="btn btn-outline-primary ms-2" _msttexthash="9814740" _msthash="28"><i class="ti ti-reload fs-6"></i></a>
							</div>
						</div>
					</div>
				</div>
							...
							<table class="table text-nowrap mb-0 align-middle">
								...
								<tr>
									<%
										...
										//////		Title Setting		//////
										//////////////////////////////////////
										String[] Title = new String[]{"번호", "아이디", "구분", "기간", "개수", "슬롯타입", "키워드", "URL", "플레이스명", "플레이스코드", "생성자", "생성일"};
								...
								<tr>
									...
									<td class="">
										<input type="text" class="form-control-plaintext text-center" id="URL_<%=logSlotList.get(i).getLOG_SLOT_IDX()%>" name="URL_<%=logSlotList.get(i).getLOG_SLOT_IDX()%>" value="<%=!"".equals(logSlotList.get(i).getPLCE_URL()) ? logSlotList.get(i).getPLCE_URL() : "-"%>" title="<%=!"".equals(logSlotList.get(i).getPLCE_URL()) ? logSlotList.get(i).getPLCE_URL() : "-"%>" readonly>
									</td>
									<td class="">
...
//				System.out.println("*********************************");
			...
			<nav class="d-flex justify-content-between pt-3" aria-label="Page navigation">
				<ul class="pagination">
					<li class="page-item">
						<% 	if(setPage > 1){ %>
						<a class="page-link d-flex border-0" href="/naver-place-log-slots/<%=setPage - 1%><%=Param%>">
				...
				<ul class="pagination d-none d-sm-block d-sm-flex">
					<%
						if(prev){
							out.print("<li class=\"page-item \"><a class=\"page-link\" href=\"/naver-place-log-slots/"+(startPage - 10)+Param+"\"> << </a></li>");
						}
						for(int i=startPage; i<=endPage; i++){
							out.print("<li class=\"page-item "+ (i==setPage ? "active" : "") +" \"><a class=\"page-link\" href=\"/naver-place-log-slots/"+(i)+Param+"\">"+(i)+"</a></li>");
						}
						if(next){
							out.print("<li class=\"page-item \"><a class=\"page-link\" href=\"/naver-place-log-slots/"+(startPage + 10)+Param+"\"> >> </a></li>");
						}
				...
				<ul class="pagination">
					<li class="page-item">
						...
						<a class="page-link d-flex border-0" href="/naver-place-log-slots/<%=setPage + 1%><%=Param%>" aria-label="Next">
